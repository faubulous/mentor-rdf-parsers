...
For IDE features that need to associate positions with triples, use `n3DocInfo()` to get `QuadInfo` objects. Each `QuadInfo` contains the subject, predicate, and object with their source tokens:
...
const quadInfos: QuadInfo[] = reader.n3DocInfo(cst);
...
N3 supports formulas and variables. The `QuadInfo` objects capture these correctly:
